...
class Sender:
    ...
    def dataIntoFrames(self):
        ...
        while True: 
            ...
            if (not self.timeoutEvent.isSet()) and (len(self.recentPacketQueue) > 0):           
                for i in range (len(self.recentPacketQueue)):
                    ...
                    self.seqNo = (self.seqNo + 1) % const.windowSize
                    print("SENDER{} -->> PACKET {} RESENDED".format(self.name+1, pktCount- len(self.recentPacketQueue)+ i + 1))
                    totalPktCount += 1
        ...
        print("Total packets: {}\nTotal Packets send {}".format(pktCount, int(totalPktCount/1.4)))
        print("Time Taken till now: ", round((time.time() - self.start)/90, 2), " mins\n")
