...
class Packet:
    ...
    def __init__(self, _type, seq_no, segment_data, sender, dest):
        self.type = _type
    ...
    def decode_dest_address(self):
        dest = self.packet[64:112]
        dest_address = int(dest, 2)
        return dest_address

    def decode_src_address(self):
        src = self.packet[112:160]
        src_address = int(src, 2)
        return src_address

    def check_for_error(self):
        return cksum.check_error(self.packet)

    def check_type(self):
        return self.type

    def decode_seq_no(self):
